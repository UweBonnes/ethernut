 ...
 * @brief  Set the Process Stack Pointer
 *
 * @param  value  Process Stack Pointer
 *
 * Assign the value to the PSP
 * (process stack pointer) Cortex processor register
 */
extern void __set_IPSR(uint32_t value);
...
/**
 * @brief  Return setting of  the interrupt line for external interrupt specified
 *
 * @param  IRQn   The positive number of the external interrupt to check.
 * @return         0 = interrupt not enabled, else interrupt is enabled.
 *
 * check if a device specific interupt in the NVIC interrupt controller is enabled.
 * The interrupt number cannot be a negative value.
 */
